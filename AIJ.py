...
###    
    ...
    def compute_N1_72(self, row):
        ...
        try:
            Pa = 9.8  # t/m²
            Cn = np.minimum(np.sqrt(Pa / sigma_v_CSR),1.7 )
            N1_72 = Cn * N72
    ...
    def compute_Na(self, row):
        """計算 Na"""
        N1_72 = self.compute_N1_72(row)  # 修正：應該是函數調用
        FC = row['FC']  # 細料含量
        ...
        try:
            ...
            if FC <= 5:
                ...
                delta_Nf = 6 + 0.2 * (FC - 10)

            elif 20 < FC <= 50:    
                delta_Nf = 8 + 0.1 * (FC - 20)    

            elif FC > 50:  # 修正3: 簡化條件判斷
                delta_Nf = 11
            else:
                delta_Nf = 0.0  # 如果 FC 為負數或其他異常情況，設為 0
    ...
    def calculate_FS(self, row, scenario='Design'):
        ...
        if pd.notna(dirt_depth) and dirt_depth != "" and dirt_depth <= 20 and dirt_depth > GWT_CSR:
            ...
            # 條件(2)：細料含量>35%但符合特殊條件
            elif FC >= 35:
                clay_content = row.get('黏土(%)', None)
                
                # 檢查黏土含量<15%或塑性指數<15
                clay_condition = False
                pi_condition = False
                
                if clay_content is not None:
                    try:
                        clay_numeric = float(clay_content)
                        clay_condition = clay_numeric < 15
                    except (ValueError, TypeError):
                        clay_condition = False
                
                try:
                    if PI != "NP" and PI is not None and PI != "":
                        pi_numeric = float(PI)
                        pi_condition = pi_numeric < 15
                except (ValueError, TypeError):
                    pi_condition = False
                
                if clay_condition or pi_condition:
                    should_evaluate = True

        not_evaluate_condition = not should_evaluate

        # 檢查是否符合任一 FS=3 的條件
        should_set_fs_3 = depth_over_20 or not_evaluate_condition
        ...
        try:
            ...
            if Na == "-" or pd.isna(Na):
                ...
                CRR = 0.2565 * ((16 * np.sqrt(Na) / 100) + (16 * np.sqrt(Na) /80) ** 14 )
    ...
    def generate_simplified_report(self, final_df: pd.DataFrame, output_dir: str = None, 
        ...
        simplified_filename = os.path.join(output_dir, f"AIJ_{scenario}_{current_date}.csv")
    ...
    def generate_lpi_summary_report(self, final_df: pd.DataFrame, output_dir: str = None) -> str:
        ...
        current_date = datetime.now().strftime("%m%d")
        filename = os.path.join(output_dir, f"LPI_Summary_AIJ_{current_date}.csv")
    ...
    def AIJ_main(self, show_gui: bool = True, input_file_path: Optional[str] = None, 
        ...
        if output_file_path is None:
            output_filename = os.path.join(output_dir, f"AIJ液化分析結果_{current_date}.csv")
        else:
...
if __name__ == "__main__":
    ...
    AIJ_analyzer = AIJ()
    AIJ_analyzer.AIJ_main(show_gui=True, input_file_path=input_path)