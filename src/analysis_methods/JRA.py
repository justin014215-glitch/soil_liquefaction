...
'''
應進行液化潛能判定之砂土層
對於沖積層之飽和土層，符合以下條件時，應按第 b 項所述方法進行土壤
液化之評估。
(1) 位於地表面下 20 公尺以內，細料含量<35%之飽和沖積砂質土層。
...
###    
    ...
    def compute_N72(self, row):
        ...
        try:
            N72 = N_value_parsed * Em_parsed / 72
            
            return format_result(N72)
        except (ValueError, TypeError):
            return "-"
        
    #計算 N1_72
    def compute_N1_72(self, row):
        """計算 N1_72"""
        N_value = row['N_value']
        sigma_v = row['累計sigmav']
        sigma_v_CSR = row['sigma_v_CSR']
        N72 = self.compute_N72(row)  
        N_value_parsed = parse_numeric_value(N_value)
        ...
        try:
            ...
            N1_72 =( 17 * N72 ) / (sigma_v_CSR + 7)
            return format_result(N1_72)
        except (ValueError, TypeError):
            return "-"
    # 計算 Na    
    def compute_Na(self, row):
        ...
        if N1_72 == "-" or pd.isna(N1_72):
            return "-"
        
        try:
            N1_72_parsed = parse_numeric_value(N1_72)
            if N1_72_parsed is None:
                return "-"
    ...
    def calculate_FS(self, row, scenario='Design'):
        ...
        # 獲取地震規模
        base_mw = row.get('基準Mw', 7.1)
        mw_value = get_scenario_mw(base_mw, scenario)
        

        # 條件1: 深度超過20公尺
        depth_over_20 = pd.notna(dirt_depth) and dirt_depth != "" and dirt_depth > 20

        # 條件2: 不符合液化評估條件
        should_evaluate = False


        # 檢查是否在評估範圍內（20m內且在地下水位以下）
        if pd.notna(dirt_depth) and dirt_depth != "" and dirt_depth <= 20 and dirt_depth > GWT_CSR:
            # JRA1996條件：
            # (1)地下水位在地表下10m以內，且飽和砂質土層在地表下20m以內
            # (2)細料含量FC≤35% 或 FC>35%且塑性指數Ip<15%
            # (3)平均粒徑D50≤10mm且有效粒徑D10≤1mm（有資料時才檢查）
            
            sandy_soils = ["SW", "SP", "SM", "SC", "GW", "GP", "GM", "GC"]
            
            # 先檢查地下水位條件（10m以內）
            gwt_condition = GWT_CSR <= 10
            
            if soil_class in sandy_soils and gwt_condition:
                # 條件(2)：細料含量檢查
                fc_condition = False
                if FC <= 35:
                    fc_condition = True
                elif FC > 35 and PI < 15:
                    fc_condition = True
                ...
                if D50 is not None and not pd.isna(D50) and D50 != "" and str(D50).strip() != "":
                    try:
                        D50_numeric = float(D50)
                        if D50_numeric > 10:  # D50 > 10mm，不符合條件
                            grain_size_condition = False
                            print(f"      D50={D50_numeric}mm > 10mm，不符合液化評估條件")
                    except (ValueError, TypeError):
                        # 無法轉換時忽略此條件
                        print(f"      D50資料無法解析，忽略此條件")
                else:
                    print(f"      D50無資料，忽略此條件")
                
                # 檢查D10（有資料才檢查）
                D10 = row.get('D10', None)
                if D10 is not None and not pd.isna(D10) and D10 != "" and str(D10).strip() != "":
                    try:
                        D10_numeric = float(D10)
                        if D10_numeric > 1:  # D10 > 1mm，不符合條件
                            grain_size_condition = False
                            print(f"      D10={D10_numeric}mm > 1mm，不符合液化評估條件")
                    except (ValueError, TypeError):
                        # 無法轉換時忽略此條件
        ...
        # 檢查必要數據完整性
        if pd.isna(depth) or pd.isna(sigma_v_csr)  or pd.isna(sigma_v):
            return {
                'Mw_used': format_result(mw_value),
                'A_value': "-",
                'SD_S': "-",
        ...
        try:
            # 1. 計算 A_value, SD_S, SM_S, Na
            A_value, SD_S, SM_S = self.compute_A_value(row, scenario)
            Na = self.compute_Na(row)
            ...
            if Na == "-" or pd.isna(Na):
                ...
                Na_numeric = float(Na)
                if Na_numeric < 14:
                    CRR = 0.0882 * np.sqrt(Na_numeric / 1.7)
                else:  # Na >= 14
                    CRR = 0.0882 * np.sqrt(Na_numeric / 1.7) + 1.6 * 10**(-6) * (Na_numeric - 14)**4.5
    ...
    def generate_simplified_report(self, final_df: pd.DataFrame, output_dir: str = None, 
        ...
        simplified_filename = os.path.join(output_dir, f"JRA_{scenario}_{current_date}.csv")
    ...
    def generate_lpi_summary_report(self, final_df: pd.DataFrame, output_dir: str = None) -> str:
        ...
        current_date = datetime.now().strftime("%m%d")
        filename = os.path.join(output_dir, f"LPI_Summary_JRA_{current_date}.csv")
    ...
    def JRA_main(self, show_gui: bool = True, input_file_path: Optional[str] = None, 
        ...
        if output_file_path is None:
            output_filename = os.path.join(output_dir, f"JRA液化分析結果_{current_date}.csv")
        else:
...
if __name__ == "__main__":
    ...
    JRA_analyzer = JRA()
    JRA_analyzer.JRA_main(show_gui=True, input_file_path=input_path)