...
液化潛能之評估。
(1) 位於地表面下 20 公尺以內，且位於地下水位以下之飽和土層。
(2) 塑性指數 Ip 小於 7 之土層。
'''
...
###    
    ...
    def is_non_liquefaction_condition(self, soil_class, PI, dirt_depth, GWT_CSR, N1_60cs):
        """判斷是否為不液化條件"""
        # 檢查塑性指數
        try:
            pi_condition = float(PI) > 7
        except (ValueError, TypeError):
            pi_condition = False
        
        # 檢查 N 值條件
        try:
            if N1_60cs is None or N1_60cs == "-" or N1_60cs == "":
                high_n_value = False
            else:
                N1_60cs_numeric = float(N1_60cs)
                high_n_value = N1_60cs_numeric >= 39
        except (ValueError, TypeError):
            high_n_value = False

        # 判斷條件
        is_clay = soil_class in ["CL", "SC", "CH", "MH"]
        above_gwt = dirt_depth <= GWT_CSR
        
        return is_clay or pi_condition or above_gwt or high_n_value
    ...
    def compute_coefficient(self, group):
        ...
        for i in range(len(group)):
            ...
            if pd.isna(density) or density == "" or density is None:
                ...
                # 進行單位轉換
                try:
                    original_value = float(density)
                    converted_value = original_value * self.unit_weight_conversion_factor
                    group.iloc[i, group.columns.get_loc(unit_weight_col)] = converted_value
                    if self.unit_weight_conversion_factor != 1.0:
                        print(f"      第{i+1}層 Density: {original_value} → {converted_value:.3f} (已轉換為 t/m³)")
                except (ValueError, TypeError):
                    group.iloc[i, group.columns.get_loc(unit_weight_col)] = 0.0
                    print(f"      第{i+1}層 Density 無法轉換，設為0")
                    missing_count += 1

        if missing_count > 0:
            print(f"    共處理 {missing_count} 個 Density 問題值")
        # 計算土層深度
        dirt_depths = []
        valid_indices = []
    ...
    def compute_N1_60cs(self, row):
        ...
        try:
            ...
            if FC <= 10:
                ...
                Ks = 1 + 0.07 * np.sqrt(FC - 10)
            
            N1_60cs = N1_60_parsed * Ks
    ...
    def compute_CRR_7_5(self, row):
        ...
        GWT_CSR = row.get('GWT_CSR', 0)  # 地下水位深度
        N1_60cs_value = row['N1_60cs']
        
        # 如果是不液化條件，直接返回 "-"
        if self.is_non_liquefaction_condition(soil_class, PI, dirt_depth, GWT_CSR, N1_60cs_value):
            return "-"
        ...
        try:
            ...
            # 使用指定公式：CRR_7_5 = 0.08 + (0.0035 × N1_60cs) / (1 - N1_60cs / 39)
            denominator = 1 - N1_60cs/ 39
            
            # 檢查分母是否為零或負數（當 N1_60cs >= 39 時）
            if N1_60cs >= 39 or denominator <= 0:
                # 當 N1_60cs >= 39 時，設定 CRR_7_5 為 "-"
                return "-"
            
            numerator = 0.0035 * N1_60cs
            CRR_7_5 = 0.08 + numerator / denominator
    ...
    def calculate_FS(self, row, scenario='Design'):
        ...
        # 新增判斷條件4: 在不液化條件內 (使用處理後的PI值)
        non_liq_condition = self.is_non_liquefaction_condition(soil_class, PI, dirt_depth, GWT_CSR, N1_60cs_value)
        if non_liq_condition:
            print(f"Debug - 不液化條件觸發: soil={soil_class}, PI={PI}, depth={dirt_depth}")
        
        # 檢查是否符合任一 FS=3 的條件
        should_set_fs_3 = depth_condition or pi_condition or crr_condition or non_liq_condition
...
if __name__ == "__main__":
    ...
    HBF_analyzer = HBF()
    HBF_analyzer.HBF_main(show_gui=True, input_file_path=input_path)